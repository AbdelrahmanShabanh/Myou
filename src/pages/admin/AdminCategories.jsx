...
export default function AdminCategories() {
  ...
  const toggleActive = async (cat) => {
    ...
    try {
      ...
      setCategories(prev => prev.map(c => c._id === cat._id ? { ...c, active: !cat.active } : c));
    } catch (err) {
              ...
              <tr key={c._id}>
                ...
                <td>
                  <button onClick={() => toggleActive(c)} style={{
                    background: c.active ? 'var(--color-success)' : 'var(--color-sold-out)',
                ...
                <td style={{textAlign: 'right'}}>
                  <button onClick={() => openEditModal(c)} className="btn btn-outline btn-sm" style={{marginRight: '8px'}}>Edit</button>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h2 style={{marginBottom: '1.5rem', fontFamily: 'Playfair Display'}}>{editId ? 'Edit Category' : 'Add Category'}</h2>
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                {image && <img src={image} alt="preview" style={{width: '60px', height: '60px', objectFit: 'cover', marginTop: '10px', borderRadius: '8px'}}/>}
              </div>
              ...
              <div className="form-group" style={{flexDirection: 'row', alignItems: 'center', gap: '8px', cursor: 'pointer'}}>
                ...
                <label htmlFor="activeCheckbox" style={{fontWeight: 500, fontSize: '14px', cursor: 'pointer'}}>Active Category</label>
              </div>
              
              <div style={{display: 'flex', gap: '1rem', marginTop: '2rem', justifyContent: 'flex-end'}}>
                <button type="button" className="btn btn-outline" onClick={() => setShowModal(false)}>Cancel</button>
                <button type="submit" className="btn btn-primary" disabled={uploading}>Save Category</button>
              </div>
            </form>
          </div>
        </div>
